...
export class Api {
  ...
  public async getBanks(countryCode: string): Promise<Bank[]> {
    try {
      const res = await this.apiInstance.get('/bank/selection/' + countryCode, { headers: this.getHeaders() }).then((res) => res);
      if (res.status === 200) {
        return res.data;
      } else {
        return [];
      ...
      if (error.response.status === 401 || error.response.status === 403) {
        if (await this.refreshAccessToken()) {
          ...
          store.dispatch({
            ...
            payload: {
              variant: AlertVariants.error,
              text: `You need to wait ${error.response.status} hours before the next refresh`,
              show: true,
              type: 'TOGGLE_BANK_ALERT',
            },
          });
